...
`mod_Tplyr_table` serves as a wrapper for displaying interactive summary tables generated with [`{Tplyr}`](https://atorus-research.github.io/Tplyr/). The module leverages Tplyr’s metadata extraction functionality to offer traceability from the summary tables back to the source listing. By clicking on a result cell in the table, the source listing will be generated. For displaying the listing the listings module of `{dv.listings}` is used, which means all functionality of that module can be leveraged.

The Tplyr table module allows multiple output tables created with `{Tplyr}` and standalone listings to be displayed under the same tab. Outputs can be selected using either a drop-down selector or separate tabs.
...
`default_vars`, `pagination`, `intended_use_label`, `receiver_id`, `review`, `footers` and `exclude_var_names_from_column_headings` are all arguments that are passed through to `{dv.listings}`. More information can be found [here](https://boehringer-ingelheim.github.io/dv.listings/)

<span style="color: red;">Note: The legacy `pagination` argument is deprecated and has been replaced by `listing_pagination`.</span>

The module also provides the following Tplyr-table specific options:

- `title_layout` - display outputs using either a drop-down menu or tabs.

- `table_pagination` - control pagination of the summary table. Can be set to 
`TRUE`, `FALSE`, or a positive integer indicating the default page size.
...
When multiple outputs are available, they can be displayed as tabs instead of using the default drop-down selector.
...
Pagination can be enabled for Tplyr summary tables using the `table_pagination` argument. 
...
<span style="color: red;">**(Important) Preserving All Levels in tplyr_table Outputs** </span>
