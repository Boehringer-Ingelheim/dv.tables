...
![](images/event_count.png)

This guide provides a detailed overview of the `event_count` module and its features. It is meant to provide guidance to
App Creators on creating Apps in DaVinci using the `event_count` module.
...
The `event_count` module makes it possible to visualize a hierarchy table with event counts for each of the levels.

# Features

`event_count` features the following tables:
...
Refer to `dv.tables::mod_event_count()` for the complete list of arguments and their description.
...
dv.manager::run_app(
  ...
  module_list = list(
    "AE Hierarchy Table" = dv.tables::mod_event_count(
      module_id = "event_count",
