# Export event group-by table tests
local({
  event_list <- list()
  ...
  test_that("Add a new row" |>
    ...
    expect_equal(exp_tab[1, 2], NA_character_)
    expect_equal(exp_tab[1, 3], 1)
    expect_equal(exp_tab[1, 21], 4)
  })
  ...
  test_that("Separates N/pct columns into two, remove parentheses" |>
    ...
    stat_cols <- expand.grid(levels(pop_list[["group"]]), levels(event_list[["evtgrp"]]))
    stat_cols <- paste(stat_cols$Var1, stat_cols$Var2, sep = "/")
    expect_true(all(paste0(stat_cols, " [N]") %in% names(exp_tab)))
    expect_true(all(paste0(stat_cols, " [%]") %in% names(exp_tab)))
  ...
  test_that("Keep N and % in single column" |>
    ...
    expect_true(all(grepl("^[0-9]+ \\([0-9.]+\\)$", tail(exp_tab[[2]], nrow(exp_tab) - 1)[1:4])))
  })
})
