...
check_mod_summary_table_auto <- function(afmm, datasets, module_id, table_dataset_name, pop_dataset_name,
    ...
    default_drop_empty_rows, default_show_category_n, default_denom, default_stats, default_collapse_method,
    default_pop_flags, default_pop_flags_after_groups, summarize_on_choices, group_by_choices, row_by_choices,
    collapse_method_choices, pop_flag_choices, total_group_val, receiver_id, err) {
    OK <- logical(0)
    used_dataset_names <- new.env(parent = emptyenv())
    OK[["module_id"]] <- CM$check_module_id("module_id", module_id, err)
    flags <- structure(list(), names = character(0))
