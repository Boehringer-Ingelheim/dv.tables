...
#'
#' @param pagination Deprecated. Legacy alias for `listing_pagination`.
#' This argument currently controls pagination for drill-down and standalone
#' listings generated by `dv.listings`. It is retained for backward compatibility
#' but will be removed in a future version. Please use `listing_pagination`
#' instead.
#'
#' @param listing_pagination Pagination configuration for the drill-down and
#' standalone listings generated by `dv.listings`. This is passed through to
#' `dv.listings::listings_server()`.
#'
#' @param table_pagination `[logical(1) | integer]`
#' Pagination behavior for the rendered Tplyr summary table. If `TRUE`, table
#' pagination is enabled using the default page size from `reactable`. If `FALSE`,
#' table pagination is disabled. If a positive whole number is supplied, table
#' pagination is enabled and the number is used as `defaultPageSize` in
#' `reactable::reactable()`. Defaults to `TRUE`.
#'
...
mod_Tplyr_table <- function(
  ...
  pagination_supplied <- !is.null(pagination)

  if (pagination_supplied) {
    warn_deprecated_pagination()
  }

  if (pagination_supplied && !is.null(listing_pagination)) {
    warning(
