...
This guide provides a detailed overview of the `mod_Tplyr_table` module
and its features. It is meant to provide guidance to App Creators on
creating Apps in DaVinci using the `mod_Tplyr_table` module.
...
`mod_Tplyr_table` serves as a wrapper for displaying interactive summary
tables generated with
[`{Tplyr}`](https://atorus-research.github.io/Tplyr/). The module
leverages Tplyr’s metadata extraction functionality to offer
...
[dv.listings](https://github.com/Boehringer-Ingelheim/dv.listings) is
...
- `output_list`: A named list defining the outputs to be generated. Each
  element of the list corresponds to a table or listing and must be a
  named list with one of the following structures:

  For summary tables:

  - `tplyr_tab_fun` A function that takes one or more datasets as input
  ...
  - `dataset_names` A character vector of dataset names required to
    generate the listing. The names of the top-level list elements are
    used as identifiers for the outputs.

- `subjid_var` Column corresponding to subject ID. Default value is
  ‘USUBJID’

**(Optional) Arguments for listing customization**

The module uses
[dv.listings](https://github.com/Boehringer-Ingelheim/dv.listings) for
showing drill-down listing data. Therefore additional arguments can be
provided for further customization.

`default_vars`, `pagination`, `intended_use_label`, `receiver_id`,
`review` are all arguments that are passed through to
[dv.listings](https://github.com/Boehringer-Ingelheim/dv.listings). more
information can be found
[here](https://boehringer-ingelheim.github.io/dv.listings/)

## Example

To better understand the set up of the module you can follow the below
example:
...
In `mod_tplyr_table` function, grouping variable levels with no data may
be omitted as unused factor levels are dropped by default. To ensure all
levels appear in the output, even those with zero counts, explicitly
define the levels of the grouping variable as factors.

Here’s an example where we ensure all levels of the `ARM` variable
